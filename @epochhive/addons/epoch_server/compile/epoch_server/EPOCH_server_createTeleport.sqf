...

{
	...
	if !(_sign isEqualTo []) then {
		_sign params ["_class",["_offset",[0,0,0]],["_texture",[]]];
		_signPos = _pos1 vectorAdd _offset;
		_sign = createVehicle[_class, _signPos, [], 0.0, "CAN_COLLIDE"];
		if !(_texture isEqualTo []) then {
			_sign setObjectTextureGlobal _texture;
		};
		_sign enableSimulationGlobal false;
		_sign allowDamage false;
		_sign setDir (_debugLocation getDir _sign);
