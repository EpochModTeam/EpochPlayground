...
class CfgEpochClient
{
    ...
    buildingJammerRange = 1000; 		// jammer range in meters
    jammerPerGroup = 9;				// allowed number of jammers per group.
    jammerGLOnly = 1;               // allow only group leader to place Jammer
    minJammerDistance = 1650;		// min distance to next Jammer
    maxBuildingHeight = 330;			// Max Height, building is allowed.
    buildingCountLimit = 2500;		//overall building limit in range of jammer (overridden if "useGroupCountLimits=1")
    ...
    useGroupCountLimits = 0;		//1 = enable leader and member counts (doesn´t affect "splitCountLimits")
    ...
    maxdoors = 80;					// Max allowed doors per Group
    maxgates = 50;					// Max allowed Gates per Group
    ...
    EPOCH_news[] = {"big event in town today","I hear Mad Dog Sherbz is going to be at the event."};
    ...
    antagonistSpawnIndex[] = {{"Epoch_Cloak_F",5},{"GreatWhite_F",5},{"Epoch_Sapper_F",20},{"Epoch_SapperG_F",1},{"Epoch_SapperB_F",5},{"I_UAV_01_F",5},{"PHANTOM",1},{"B_Heli_Transport_01_F",1},{"EPOCH_RyanZombie_1",36},{"I_Soldier_EPOCH",3}}; // {"type", limit}
    customVarsDefaults[] = {{"Temp",98.6,{106.7,95,102,105,96,95}},{"Hunger",5000,{5000,0,5001,5001,1250,0}},{"Thirst",2500,{2500,0,2501,2501,625,0}},{"AliveTime",0,{-2,0}},{"Energy",0,{2500,0}},{"Wet",0,{100,0,35,55,-1,-1}},{"Soiled",0,{100,0,35,55,-1,-1}},{"Immunity",0,{100,0}},{"Toxicity",0,{100,0,35,55,-1,-1}},{"Stamina",100,{"EPOCH_playerStaminaMax",0}},{"Crypto",0,{250000,0}},{"HitPoints",{0,0,0,0},{1,0,0.5,1,-1,-1}},{"BloodP",100,{190,0,120,140,70,50}},{"SpawnArray",{},{}},{"Karma",0,{50000,-50000}},{"Alcohol",0,{100,0,35,55,-1,-1}},{"Radiation",0,{100,0,35,55,-1,-1}},{"Nuisance",0,{100,0}},{"MissionArray",{},{}}}; // EPOCH_player + varName, default value, {max,min,high-warn,high-critical,low-warn,low-critical}
